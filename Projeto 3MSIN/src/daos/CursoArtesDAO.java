...
public class CursoArtesDAO {
	...
	public void incluir(CursoArtesTO to) {
		...
		String sqlInsert = "INSERT INTO Artes (codigoCursoArt, nomeCurso, dataInicio, dataTermino, horarioPrevisto, numeroDeVagas, valorCurso, livros, descricaoMaterial ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

		try (Connection conn = ConnectionFactory.obtemConexao();
				PreparedStatement st = conn.prepareStatement(sqlInsert);) {

			st.setInt(1, to.getCodigoArtes());
			st.setString(2, to.getNomeArtes());
			...
			st.setString(8, to.getLivros());
			st.setString(9, to.getDescricaoMaterial());
			st.execute();
