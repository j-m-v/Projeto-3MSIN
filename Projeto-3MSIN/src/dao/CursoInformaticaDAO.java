...
public class CursoInformaticaDAO {
	...
	public void incluir(CursoInformaticaTO to) {
		String sqlInsert = "INSERT INTO Informatica (nomeCurso, dataInicio, dataTermino, horarioPrevisto, numeroDeVagas, valorCurso, descricaoSoftware, numeroLaboratorios ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			try(PreparedStatement stm1 = conn.prepareStatement(sqlSelect);
					...
					if(rs.next()){
						to.setCodigoInformatica(rs.getInt(1));
					}
			}
	...
	public void alterar(CursoInformaticaTO to) {
		...
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			st.setString(7, to.getSoftwares());
			st.setString(8, to.getNumeroLaboratorios());
			st.setInt(9, to.getCodigoInformatica());
			st.execute();
		} catch (SQLException e) {
			e.printStackTrace();
		}
